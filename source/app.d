...
public:
	...
	this(string dir, string serialPath){
		...
		version(masterhacker){
			// make all text in log and input green, so people thing we're "hAcKiNg"
			_log.textColor = ALT_FG;
			_commandInput.textColor = ALT_FG;
			// </masterhacker>
		}
		// arrange the elements
		_infoLayout.addWidget([_labelToggleTime, _labelRpm, _labelDegLost, _labelFps]);
		_term.addWidget([_infoLayout, _log, _progBar, _commandInput]);
		// set progress bar
